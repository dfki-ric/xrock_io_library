...
namespace xrock_io_library
{
    ...
    RestDB::RestDB(const std::string& dbAddress, const std::string& graph) : registry(new xtypes::ProjectRegistry())
    {
    ...
    std::vector<std::pair<std::string, std::string>> RestDB::requestModelListByDomain(const std::string &domain)
    {
        ...
        const std::vector<XTypePtr> models = client->find(ComponentModel::classname, props);
                                   ...
                                   const bool limit)
    {
        ...
        std::vector<XTypePtr> xtypes = client->find(ComponentModel::classname, props);
        if (xtypes.size() == 0)
        {
    ...
    bool RestDB::buildModule(const std::string &uri, const std::string &moduleName, const std::map<std::string, std::string> &selectedImplementationsUris)
    {
        // Load the top level cm to build module for
        xtypes::XTypePtr xtype = client->load(uri, ComponentModel::classname);
    ...
    ConfigMap RestDB::getUnresolvedAbstracts(const std::string &uri)
    {
        xtypes::XTypePtr xtype = client->load(uri, ComponentModel::classname);
        xtypes::ComponentModelPtr top_lvl_cm = std::static_pointer_cast<ComponentModel>(xtype);
